...
async function loadDataFromBackend(dataType, limit = 50) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Для отзывов, заказов и заказов чата возвращаем все данные, для остальных - ограниченное количество
            let paginatedData;
            if (dataType === 'reviews' || dataType === 'requests' || dataType === 'chat_orders') {
                paginatedData = allData; // Все отзывы, заказы и заказы чата
            } else {
...
async function updateDataWithFullReplace(dataType) {
    try {
        // Для отзывов, заказов и заказов чата получаем все данные, для остальных - ограниченное количество
        const limit = (dataType === 'reviews' || dataType === 'requests' || dataType === 'chat_orders') ? 1000 : 100;
...
function displayData(dataType, data) {
    switch (dataType) {
        ...
        case 'chat_orders':
            ...
            console.log(`🔍 Обработка заказов для чата: ${data.length} записей`);
            
            // Фильтруем заказы по текущему пользователю
            const chatCurrentUserData = window.userData || userData;
            const chatCurrentUserId = chatCurrentUserData?.id;
            
            console.log('🔍 Фильтрация заказов чата:', {
                totalOrders: data.length,
                currentUserData: chatCurrentUserData,
                currentUserId: chatCurrentUserId,
                currentUserIdType: typeof chatCurrentUserId
            });
            
            // Фильтруем заказы по пользователю
            let userChatOrders = [];
            let chatTargetUserId = chatCurrentUserId;
            
            // Если ID пользователя не определен или равен 'unknown', показываем только заказы с user_id: 'unknown'
            if (!chatTargetUserId || chatTargetUserId === 'undefined' || chatTargetUserId === 'null' || chatTargetUserId === '' || chatTargetUserId === 'unknown') {
                console.log(`🔍 ID пользователя не определен или равен 'unknown', показываем только заказы с user_id: 'unknown'`);
                userChatOrders = data.filter(order => {
                    const orderUserId = String(order.user_id || '');
                    return orderUserId === 'unknown';
                });
            } else {
                // Фильтруем только заказы текущего пользователя
                userChatOrders = data.filter(order => {
                    const orderUserId = String(order.user_id || '');
                    const targetUserIdStr = String(chatTargetUserId || '');
            ...
            console.log(`✅ Обработано ${Object.keys(chatOrders).length} заказов для чата`);
            ...
            break;
        case 'chat_messages':
            // Обрабатываем данные сообщений чата
            console.log(`🔍 Обработка сообщений чата: ${data.length} записей`);
            ...
            console.log(`✅ Обработано ${data.length} сообщений чата для ${Object.keys(groupedMessages).length} заказов`);
            
            // Обновляем отображение сообщений если чат открыт
            if (currentChat) {
        ...
        case 'requests':
            ...
            // Если ID пользователя не определен или равен 'unknown', показываем только заказы с user_id: 'unknown'
            if (!targetUserId || targetUserId === 'undefined' || targetUserId === 'null' || targetUserId === '' || targetUserId === 'unknown') {
                console.log(`🔍 ID пользователя не определен или равен 'unknown', показываем только заказы с user_id: 'unknown'`);
            ...
            console.log(`🔍 Фильтрация заказов: найдено ${userOrders.length} заказов (всего: ${data.length})`);
...
function updateChatDisplay() {
    // Обновляем теги заказов
    updateChatOrderTags();
    
    // Обновляем сообщения если есть активный чат
    if (currentChat) {
        loadChatMessages(currentChat);
    }
}
...
function loadServiceCards() {
    ...
    const servicesData = {
        'aiTelegram': {
            ...
            price: getServicePrice('aiTelegram', 'от 20,000₽'),
            button: 'Подробнее'
        },
        'channelAutomation': {
            title: 'Автоматизация каналов',
            description: 'Системы для автоматического ведения Telegram-каналов',
            price: getServicePrice('channelAutomation', 'от 10,000₽/мес'),
            button: 'Подробнее'
        },
        'onboardingSystems': {
            title: 'Системы онбординга',
            description: 'Автоматизированные системы адаптации новых сотрудников',
            price: getServicePrice('onboardingSystems', 'от 30,000₽'),
            button: 'Подробнее'
        },
        'socialManagement': {
            title: 'Ведение социальных сетей',
            description: 'Ведение соцсетей ВК, Телеграм. Придумываю посты по тематике, пишу и выкладываю. Анализирую реакции пользователей для создания более интересного контента.',
            price: getServicePrice('socialManagement', 'от 10,000₽/мес'),
            button: 'Подробнее'
        },
        'productSupport': {
            title: 'Продуктовое сопровождение',
            description: 'Стратегическое управление продуктом на всех этапах жизненного цикла',
            price: getServicePrice('productSupport', 'от 180,000₽/мес'),
            button: 'Подробнее'
        },
        'noCodeWebsites': {
            title: 'Сайты и веб-приложения',
            description: 'No-code, vibe-coding разработка',
            price: getServicePrice('noCodeWebsites', 'от 25,000₽'),
            button: 'Подробнее'
        },
        'customDevelopment': {
        ...
        'telegramBots': {
            ...
            price: getServicePrice('telegramBots', 'от 20,000₽'),
            button: 'Подробнее'
        },
        'projectManager': {
            title: 'Проектный менеджер',
            description: 'Управление проектами с фокусом на результатах и эффективности команды',
            price: getServicePrice('projectManager', 'от 180,000₽/мес'),
            button: 'Подробнее'
        },
        'businessDevelopmentManager': {
            title: 'Менеджер по развитию бизнеса',
            description: 'Развитие бизнеса через оптимизацию процессов и внедрение новых технологий',
...
function getModalData(serviceKey) {
    const modalData = {
        'aiTelegram': {
            ...
            technologies: ['Telegram Bot API', 'Python', 'AI модели', 'Хранение данных'],
            price: getServicePrice('aiTelegram', 'от 20,000₽'),
        ...
        'channelAutomation': {
            ...
            technologies: ['Telegram API', 'Python', 'Автоматизация', 'Планирование'],
            price: getServicePrice('channelAutomation', 'от 10,000₽/мес'),
            duration: 'от 2 недель'
        },
        'onboardingSystems': {
            title: 'Системы онбординга',
            description: 'Автоматизированные системы адаптации новых сотрудников для быстрого ввода в должность.',
            features: [
                'Индивидуальная разработка',
                'Прогресс-трекинг адаптации',
                'Автоматические напоминания о задачах',
                'Тестирование знаний и навыков',
                'Персонализация обучения под должность',
                'Быстрая настройка под ваши процессы'
            ],
            technologies: ['Системы обучения', 'Контроль качества', 'Масштабируемость', 'Автоматизация'],
            price: getServicePrice('onboardingSystems', 'от 30,000₽'),
            duration: 'от 2 недель'
        },
        'socialManagement': {
            ...
            description: 'Ведение соцсетей ВК, Телеграм. Придумываю посты по тематике, пишу и выкладываю. Анализирую реакции пользователей для создания более интересного контента.',
            features: [
                'Ведение ВКонтакте и Telegram',
                'Создание постов по тематике',
                'Написание и публикация контента',
                'Анализ реакций пользователей',
                'Адаптация контента под аудиторию',
                'Регулярное обновление страниц'
            ],
            technologies: ['ВКонтакте', 'Telegram', 'Аналитика', 'Контент-маркетинг'],
            price: getServicePrice('socialManagement', 'от 10,000₽/мес'),
            duration: 'Постоянно'
        },
        'productSupport': {
            title: 'Продуктовое сопровождение',
            description: 'Управление продуктом на основе данных и метрик. Стратегические решения, подкрепленные аналитикой и измеримыми результатами.',
            features: [
                'Анализ продукта и оптимизация функций',
                'Разработка продуктовой стратегии и позиционирования',
                'Создание и управление продуктовым roadmap',
                'Аналитика пользовательского поведения и метрик',
                'Координация cross-functional команд',
                'Планирование и реализация стратегии роста'
            ],
            technologies: ['Аналитика продукта', 'Метрики продукта', 'Управление'],
            results: [
                'Улучшение опыта клиентов (Customer Experience)',
                'Снижение оттока клиентов (Churn Rate)',
                'Повышение ценности клиента (Customer Lifetime Value)',
                'Рост удержания клиентов (Retention Rate)',
                'Оптимизация бизнес-процессов (Process Optimization)'
            ],
            price: getServicePrice('productSupport', 'от 180,000₽/мес'),
            duration: 'Постоянно'
        },
        'projectManager': {
            title: 'Проектный менеджер',
            description: 'Управление проектами через оптимизацию процессов и координацию команд. Фокус на достижении результатов в срок.',
            features: [
                'Планирование проекта и постановка целей',
                ...
                'Управление изменениями и адаптация планов',
                'Коммуникация со стейкхолдерами и отчетность'
            ],
            technologies: ['Agile', 'Scrum', 'Kanban', 'Project Analytics', 'Performance Metrics'],
            results: [
                'Соблюдение сроков проекта (On-Time Delivery)',
                'Качество результата (Quality Assurance)',
                'Эффективность команды (Team Efficiency)'
            ],
            price: getServicePrice('projectManager', 'от 180,000₽/мес'),
            duration: 'По проекту'
        },
        'businessDevelopmentManager': {
            title: 'Менеджер по развитию бизнеса',
            description: 'Решаю ключевые проблемы бизнес-развития: оптимизация процессов, внедрение технологий, стратегический рост. Каждое решение направлено на результат.',
            features: [
                'Анализ текущих бизнес-процессов и выявление узких мест',
                'Оптимизация операционных процессов и повышение эффективности',
                'Внедрение новых технологий и автоматизация рутины',
                'Стратегическое планирование роста и масштабирования',
                'Анализ метрик и KPI для принятия решений',
                'Координация с командой и внедрение изменений'
            ],
            technologies: ['Бизнес-аналитика', 'Анализ процессов', 'Автоматизация', 'Управление проектами'],
            results: [
                'Повышение операционной эффективности (Operational Efficiency)',
                'Ускорение бизнес-процессов (Process Speed)',
                'Улучшение качества процессов (Process Quality)',
                'Рост производительности команды (Team Productivity)'
            ],
            price: getServicePrice('businessDevelopmentManager', 'от 180,000₽/мес'),
            duration: 'Постоянно'
        },
        'noCodeWebsites': {
            title: 'Сайты и веб-приложения',
            description: 'Создаю сайты и веб-приложения используя no-code и AI-разработку. От лендингов до мини-приложений для Telegram.',
            features: [
                'Лендинги и сайты',
                'Веб приложения',
                'Мини приложения в Телеграм',
                'Интеграции',
                'Адаптивный дизайн'
            ],
            technologies: ['No-code', 'Vibe-coding', 'Telegram Mini Apps'],
            price: getServicePrice('noCodeWebsites', 'от 25,000₽'),
            duration: 'от 2 недель'
        },
        'telegramBots': {
        ...
        modalContent.innerHTML = `
          <div class="service-modal">
            ...
            <div class="service-details">
              ...
              ${modalData.results ? `
              <div class="detail-item">
                <h4>Результаты для бизнеса:</h4>
                <ul class="results-list">
                  ${modalData.results.map(r=>`<li><i class="fas fa-chart-line"></i>${r}</li>`).join('')}
                </ul>
              </div>
              ` : ''}
            </div>
...
function openTagModal(tagType) {
    const tagData = {
        'product-manager': {
            ...
            description: 'Управляю продуктами на основе данных и метрик. Стратегические решения, подкрепленные аналитикой и измеримыми результатами.',
            details: [
                'Анализ продукта и оптимизация функций',
                'Разработка продуктовой стратегии',
                'Создание и управление roadmap',
                'Аналитика пользовательского поведения',
                'Координация cross-functional команд',
                'Планирование стратегии роста'
            ],
            icon: 'fas fa-project-diagram'
        },
        'automation': {
            title: 'Автоматизация',
            description: 'Создаю системы автоматизации для бизнес-процессов. От Telegram ботов до автоматизации каналов и систем онбординга.',
            details: [
                'AI-ассистенты для Telegram',
        ...
        'ai-expert': {
            title: 'AI Эксперт',
            description: 'Использую AI как инструмент для улучшения бизнеса. Создаю интеллектуальных помощников и автоматизирую процессы.',
            details: [
                'AI-ассистенты для Telegram',
        ...
        'analytics': {
            title: 'Аналитика',
            description: 'Анализирую данные для принятия стратегических решений. От продуктовой аналитики до бизнес-метрик и оптимизации процессов.',
            details: [
        ...
        'no-code': {
            title: 'No-Code',
            description: 'Создаю сайты и веб-приложения без написания кода. Использую современные платформы и AI-разработку.',
            details: [
                'Лендинги и сайты',
                'Веб-приложения',
                'Telegram Mini Apps',
        ...
        'startups': {
            title: 'Стартапы',
            description: 'Помогаю стартапам с MVP, стратегией развития и автоматизацией процессов. Опыт создания проектов и стартапов.',
            details: [
                'MVP разработка',
...
function updateChatOrderTags() {
    ...
    if (!orderTags) {
        console.log('orderTags element not found');
        return;
    }
    ...
    console.log('updateChatOrderTags: chatOrders', chatOrders);
    console.log('updateChatOrderTags: orderIds', orderIds);
    console.log('updateChatOrderTags: orderTags element', orderTags);
    console.log('updateChatOrderTags: currentChat', currentChat);
    
    if (orderIds.length === 0) {
        console.log('❌ updateChatOrderTags: нет заказов для отображения');
...
function loadOrderTags() {
    ...
    // Используем новую функцию updateChatOrderTags
    updateChatOrderTags();
}
